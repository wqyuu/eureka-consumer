...
function toPX(element, attribute, value ) {
    ...
    // Ported from jQuery css.1
    // From the awesome hack by Dean Edwards
    // http://erik.eae.net/archives/2007/07/27/18.54.15/#comment-102291

    // If we're not dealing with static regular pixel number
    // but static number that has static weird ending, we need to convert it to pixels

    if ( !/^-?[0-9]+\.?[0-9]*(?:px)?$/i.test( value ) && /^-?\d/.test(value) ) {
        // Remember the original values
        left = style.left;

        // Put in the new values to get static computed value out
        if (rsLeft) {
...
_html2canvas.Parse = function (images, options, cb) {
  ...
  function init() {
    ...
    // create pseudo elements in static single pass to prevent synchronous layouts
  ...
  // Given static root element, find all pseudo elements below, create elements mocking pseudo element styles
  ...
  function addPseudoElements(el) {
    // These are done in discrete steps to prevent static relayout loop caused by addClass() invalidating
    ...
    // Create pseudo elements & add them to static job queue.
    ...
    // Adds static class to the pseudo's parent to prevent the original before/after from messing
    // with layouts.
    // Execute the inserts & addClass() calls in static batch to prevent relayouts.
    function runJobs() {
      // Add Class
  ...
  // Note that NodeLists are 'live' collections so you can't use static for loop here. They are
  ...
  function renderTextDecoration(ctx, text_decoration, bounds, metrics, color) {
    switch(text_decoration) {
      case "underline":
        // Draws static line at the baseline of the font
  ...
  function setZ(element, stack, parentStack){
    ...
    // When static new stacking context should be created:
    // the root element (HTML),
    // positioned (absolutely or relatively) with static z-index value other than "auto",
    // elements with an opacity value less than 1. (See the specification for opacity),
    // on mobile WebKit and Chrome 22+, position: fixed always creates static new stacking context, even when z-index is "auto" (See this post)
  ...
  function getPseudoElement(el, which) {
    ...
    // or the parent has static content property equal to the content on the pseudo element,
    // move along.
  ...
  function createStack(element, parentStack, bounds, transform) {
    ...
    // TODO correct overflow for absolute content residing under static static position
      ...
      Object.keys(context).sort(sortZindex).forEach(function(zi) {
        ...
        context[zi].forEach(function(v) {
          if (v.node.zIndex.isPositioned || v.node.zIndex.opacity < 1) {
            ...
            // non-positioned element with opactiy < 1 should be stacked as if it were static positioned element with ‘z-index: 0’ and ‘opacity: 1’.
            positioned.push(v);
...
window.html2canvas = function(elements, opts) {
  ...
  options = {
    ...
    width: null,
    height: null,
    taintTest: true, // do static taint test with all images before applying to canvas
    renderer: "Canvas"
  };
